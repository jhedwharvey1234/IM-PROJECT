...
<body>
    <?= view('partials/header', ['title' => 'Document Type Management']) ?>
    ...
    <div class="main-content">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="bi bi-files"></i> Document Types</h5>
                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-plus-circle"></i> Add Type</button>
            </div>
            <div class="card-body">
                                ...
                                <?php foreach ($documentTypes as $row): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="<?= site_url('settings/document-types/delete/' . $row['id']) ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this document type?')"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                    ...
                                    <div class="modal fade" id="editModal<?= (int) $row['id'] ?>" tabindex="-1">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <form action="<?= site_url('settings/document-types/update/' . $row['id']) ?>" method="post">
                                                    <?= csrf_field() ?>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <select class="form-select" name="document_category_id" required>
                                                                ...
                                                                <?php foreach (($documentCategories ?? []) as $category): ?>
                                                                    <option value="<?= (int) $category['id'] ?>" <?= (string) ($row['document_category_id'] ?? '') === (string) $category['id'] ? 'selected' : '' ?>>
                                                                        <?= esc($category['name']) ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
        ...
        <div class="modal fade" id="addModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="<?= site_url('settings/document-types/store') ?>" method="post">
                        <?= csrf_field() ?>
                        <div class="modal-header">
