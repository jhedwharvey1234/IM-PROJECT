...
<body>
    <?= view('partials/header', ['title' => 'Create User Role']) ?>
    ...
    <div class="main-content">
        ...
        <div style="background: white; border-radius: 5px; padding: 20px; max-width: 720px;">
            <h4 class="mb-3"><i class="bi bi-person-badge"></i> Create User Role</h4>
            <form action="<?= site_url('settings/user-roles/store') ?>" method="post">
                <?= csrf_field() ?>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="role_key" value="<?= old('role_key') ?>" required pattern="[A-Za-z0-9_-]+" title="Use letters, numbers, underscores, or dashes.">
                    <small class="text-muted">Stored in users.usertype (example: readonly, superadmin).</small>
                </div>
                <div class="mb-3">
                    <label class="form-label">Description</label>
                    <textarea class="form-control" name="description" rows="3"><?= old('description') ?></textarea>
                </div>
                <button type="submit" class="btn btn-success"><i class="bi bi-check-circle"></i> Create Role</button>
