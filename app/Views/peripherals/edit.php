...
<body>
    ...
    <div class="main-content">
        ...
        <div class="breadcrumb-nav">
            ...
            <span class="separator">›</span>
            <span class="current">Edit Peripheral: <?= !empty($peripheral['model']) ? esc($peripheral['model']) : 'Peripheral #' . $peripheral['id'] ?></span>
        </div>
        ...
        <form action="<?= site_url('peripherals/update/' . $peripheral['id']) ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="section-card">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="peripheral_type_id" name="peripheral_type_id" required>
                            ...
                            <?php foreach ($peripheral_types as $id => $type_name): ?>
                                <option value="<?= $id ?>" <?= old('peripheral_type_id', $peripheral['peripheral_type_id']) == $id ? 'selected' : '' ?>><?= $type_name ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="qty" name="qty" min="1" value="<?= old('qty', $peripheral['qty'] ?? '1') ?>">
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="asset_id" name="asset_id">
                            ...
                            <?php foreach ($assets as $id => $asset_info): ?>
                                <option value="<?= $id ?>" <?= old('asset_id', $peripheral['asset_id'] ?? '') == $id ? 'selected' : '' ?>><?= $asset_info ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="unit_id" name="unit_id">
                            ...
                            <?php foreach ($units as $id => $unitName): ?>
                                <option value="<?= $id ?>" <?= old('unit_id', $peripheral['unit_id'] ?? '') == $id ? 'selected' : '' ?>><?= esc($unitName) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <?php if (!empty($peripheral['device_image'])): ?>
                            <div class="mb-2">
                                <img src="<?= base_url('uploads/devices/' . $peripheral['device_image']) ?>" alt="device" style="max-height:120px; max-width:200px;" class="rounded border">
                            </div>
                        <?php endif; ?>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" class="form-control" id="purchase_cost" name="purchase_cost" value="<?= old('purchase_cost', $peripheral['purchase_cost'] ?? '') ?>" placeholder="0.00">
                        </div>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="department_id" name="department_id">
                            ...
                            <?php foreach ($departments as $id => $department_name): ?>
                                <option value="<?= $id ?>" <?= old('department_id', $peripheral['department_id']) == $id ? 'selected' : '' ?>><?= $department_name ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="assigned_to_user_id" name="assigned_to_user_id">
                            ...
                            <?php foreach ($assignable_users as $id => $full_name): ?>
                                <option value="<?= $id ?>" <?= old('assigned_to_user_id', $peripheral['assigned_to_user_id']) == $id ? 'selected' : '' ?>><?= $full_name ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="row g-2 mt-5">
                <div class="col-12">
                    <button type="submit" class="btn btn-primary btn-lg">
                        <i class="bi bi-check-circle"></i> Update Peripheral
                    </button>
    ...
    <script>
        ...
        const selectedWorkstationId = <?= json_encode((int)$peripheral['workstation_id']) ?>;
        ...
        locationSelect.addEventListener('change', function () {
            ...
            if (selectedLocationId) {
                ...
                filteredWorkstations.forEach(ws => {
                    ...
                    if (ws.id === selectedWorkstationId) {
