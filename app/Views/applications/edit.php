...
<body>
    <?= view('partials/header', ['title' => 'Edit Application']) ?>
    ...
    <div class="main-content">
        ...
        <div class="breadcrumb-nav">
            ...
            <span class="separator">›</span>
            <span class="current">Edit: <?= !empty($application['app_name']) ? esc($application['app_name']) : 'Application #' . $application['id'] ?></span>
        </div>
        ...
        <form action="<?= site_url('applications/update/' . $application['id']) ?>" method="post" class="needs-validation">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="app_category" name="app_category" 
                               value="<?= old('app_category', $application['app_category']) ?>" maxlength="100" placeholder="e.g., Core System, Support, Reporting">
                    </div>
                    <div class="col-md-6">
                        <label for="app_type" class="form-label">Application Type</label>
                        <select class="form-select" id="app_type" name="app_type">
                            <option value="">Select Type</option>
                            <option value="Web" <?= old('app_type', $application['app_type']) === 'Web' ? 'selected' : '' ?>>Web</option>
                            <option value="Mobile" <?= old('app_type', $application['app_type']) === 'Mobile' ? 'selected' : '' ?>>Mobile</option>
                            <option value="Desktop" <?= old('app_type', $application['app_type']) === 'Desktop' ? 'selected' : '' ?>>Desktop</option>
                            <option value="API" <?= old('app_type', $application['app_type']) === 'API' ? 'selected' : '' ?>>API</option>
                            <option value="Batch" <?= old('app_type', $application['app_type']) === 'Batch' ? 'selected' : '' ?>>Batch</option>
                            <option value="SaaS" <?= old('app_type', $application['app_type']) === 'SaaS' ? 'selected' : '' ?>>SaaS</option>
                        </select>
                    </div>
                    <div class="col-12">
                        <label for="description" class="form-label">Description</label>
                        <textarea class="form-control" id="description" name="description" 
                                  rows="3" placeholder="Enter application description..."><?= old('description', $application['description']) ?></textarea>
                    </div>
                    <div class="col-12">
                        ...
                        <textarea class="form-control" id="business_purpose" name="business_purpose" 
                                  rows="3" placeholder="Detailed business purpose..."><?= old('business_purpose', $application['business_purpose']) ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="section-card">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        <small class="text-muted d-block mb-2"><i class="bi bi-info-circle"></i> <?= count($contacts) ?> contact(s) recorded</small>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="url" class="form-control" id="production_url" name="production_url" 
                               value="<?= old('production_url', $application['production_url']) ?>" maxlength="255" placeholder="https://..." pattern="https?://.+" title="Please enter a valid URL starting with http:// or https://">
                    ...
                    <div class="col-12">
                        <small class="text-muted d-block"><i class="bi bi-info-circle"></i> <?= count($environments) ?> environment(s) configured</small>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="server_type" name="server_type">
                            <option value="">Select Type</option>
                            <option value="VM" <?= old('server_type', $infrastructure['server_type'] ?? '') === 'VM' ? 'selected' : '' ?>>VM</option>
                            <option value="Physical" <?= old('server_type', $infrastructure['server_type'] ?? '') === 'Physical' ? 'selected' : '' ?>>Physical</option>
                            <option value="Container" <?= old('server_type', $infrastructure['server_type'] ?? '') === 'Container' ? 'selected' : '' ?>>Container</option>
                            <option value="Cloud" <?= old('server_type', $infrastructure['server_type'] ?? '') === 'Cloud' ? 'selected' : '' ?>>Cloud</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="database_type" name="database_type">
                            ...
                            <option value="MySQL" <?= old('database_type', $infrastructure['database_type'] ?? '') === 'MySQL' ? 'selected' : '' ?>>MySQL</option>
                            <option value="PostgreSQL" <?= old('database_type', $infrastructure['database_type'] ?? '') === 'PostgreSQL' ? 'selected' : '' ?>>PostgreSQL</option>
                            <option value="Oracle" <?= old('database_type', $infrastructure['database_type'] ?? '') === 'Oracle' ? 'selected' : '' ?>>Oracle</option>
                            <option value="MSSQL" <?= old('database_type', $infrastructure['database_type'] ?? '') === 'MSSQL' ? 'selected' : '' ?>>MSSQL</option>
                            <option value="MongoDB" <?= old('database_type', $infrastructure['database_type'] ?? '') === 'MongoDB' ? 'selected' : '' ?>>MongoDB</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="database_server" name="database_server" 
                               value="<?= old('database_server', $infrastructure['database_server'] ?? '') ?>" maxlength="100">
                    </div>
                    <div class="col-md-6">
                        <label for="storage_location" class="form-label">Storage Location</label>
                        <input type="text" class="form-control" id="storage_location" name="storage_location" 
                               value="<?= old('storage_location', $infrastructure['storage_location'] ?? '') ?>" maxlength="255" placeholder="e.g., /var/www, NAS">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="backup_location" name="backup_location" 
                               value="<?= old('backup_location', $infrastructure['backup_location'] ?? '') ?>" maxlength="255">
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="programming_language" name="programming_language" 
                               value="<?= old('programming_language', $techDetails['programming_language'] ?? '') ?>" maxlength="100" placeholder="e.g., PHP, Python, Java">
                    </div>
                    <div class="col-md-6">
                        <label for="framework" class="form-label">Framework</label>
                        <input type="text" class="form-control" id="framework" name="framework" 
                               value="<?= old('framework', $techDetails['framework'] ?? '') ?>" maxlength="100" placeholder="e.g., Laravel, Spring Boot">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="frontend_technology" name="frontend_technology" 
                               value="<?= old('frontend_technology', $techDetails['frontend_technology'] ?? '') ?>" maxlength="100" placeholder="e.g., React, Vue, Bootstrap">
                    </div>
                    <div class="col-md-6">
                        <label for="middleware" class="form-label">Middleware</label>
                        <input type="text" class="form-control" id="middleware" name="middleware" 
                               value="<?= old('middleware', $techDetails['middleware'] ?? '') ?>" maxlength="255" placeholder="e.g., Apache, Nginx, Tomcat">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="container_technology" name="container_technology" 
                               value="<?= old('container_technology', $techDetails['container_technology'] ?? '') ?>" maxlength="100" placeholder="e.g., Docker, Kubernetes">
                    </div>
                    <div class="col-12">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="url" class="form-control" id="repository_url" name="repository_url" 
                               value="<?= old('repository_url', $application['repository_url']) ?>" maxlength="255" placeholder="https://github.com/..." pattern="https?://.+" title="Please enter a valid URL starting with http:// or https://">
                        <small class="url-hint-invalid text-danger">Please enter a valid URL starting with http:// or https://</small>
                    </div>
                    <?php if (!empty($dependencies)): ?>
                    <div class="col-12">
                        <small class="text-muted d-block"><i class="bi bi-info-circle"></i> <?= count($dependencies) ?> dependenc(ies) recorded</small>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="data_classification" name="data_classification">
                            ...
                            <option value="Public" <?= old('data_classification', $application['data_classification']) === 'Public' ? 'selected' : '' ?>>Public</option>
                            <option value="Internal" <?= old('data_classification', $application['data_classification']) === 'Internal' ? 'selected' : '' ?>>Internal</option>
                            <option value="Confidential" <?= old('data_classification', $application['data_classification']) === 'Confidential' ? 'selected' : '' ?>>Confidential</option>
                            <option value="Sensitive" <?= old('data_classification', $application['data_classification']) === 'Sensitive' ? 'selected' : '' ?>>Sensitive</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <div class="form-check mt-4">
                            <input class="form-check-input" type="checkbox" id="personal_data_flag" name="personal_data_flag" 
                                   value="1" <?= old('personal_data_flag', $application['personal_data_flag']) ? 'checked' : '' ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="authentication_type" name="authentication_type">
                            <option value="">Select Type</option>
                            <option value="SSO" <?= old('authentication_type', $application['authentication_type']) === 'SSO' ? 'selected' : '' ?>>SSO</option>
                            <option value="AD" <?= old('authentication_type', $application['authentication_type']) === 'AD' ? 'selected' : '' ?>>Active Directory</option>
                            <option value="OAuth" <?= old('authentication_type', $application['authentication_type']) === 'OAuth' ? 'selected' : '' ?>>OAuth</option>
                            <option value="LDAP" <?= old('authentication_type', $application['authentication_type']) === 'LDAP' ? 'selected' : '' ?>>LDAP</option>
                            <option value="Internal" <?= old('authentication_type', $application['authentication_type']) === 'Internal' ? 'selected' : '' ?>>Internal</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <div class="form-check mt-4">
                            <input class="form-check-input" type="checkbox" id="encryption_enabled" name="encryption_enabled" 
                                   value="1" <?= old('encryption_enabled', $application['encryption_enabled']) ? 'checked' : '' ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="last_security_review" name="last_security_review" 
                               value="<?= old('last_security_review', $application['last_security_review']) ?>">
                    </div>
                    <div class="col-md-6">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="lifecycle_stage" name="lifecycle_stage">
                            ...
                            <option value="Development" <?= old('lifecycle_stage', $application['lifecycle_stage']) === 'Development' ? 'selected' : '' ?>>Development</option>
                            <option value="Active" <?= old('lifecycle_stage', $application['lifecycle_stage']) === 'Active' ? 'selected' : '' ?>>Active</option>
                            <option value="Maintenance" <?= old('lifecycle_stage', $application['lifecycle_stage']) === 'Maintenance' ? 'selected' : '' ?>>Maintenance</option>
                            <option value="Sunset" <?= old('lifecycle_stage', $application['lifecycle_stage']) === 'Sunset' ? 'selected' : '' ?>>Sunset</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="eol_date" name="eol_date" 
                               value="<?= old('eol_date', $application['eol_date']) ?>">
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="text" class="form-control" id="replacement_system" name="replacement_system" 
                               value="<?= old('replacement_system', $application['replacement_system']) ?>" maxlength="255">
                    </div>
                    <div class="col-12">
                        ...
                        <textarea class="form-control" id="upgrade_roadmap" name="upgrade_roadmap" 
                                  rows="3" placeholder="Planned upgrades and timeline..."><?= old('upgrade_roadmap', $application['upgrade_roadmap']) ?></textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="availability_sla" name="availability_sla" 
                               value="<?= old('availability_sla', $application['availability_sla']) ?>" placeholder="e.g., 99.9%">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="monitoring_tool" name="monitoring_tool" 
                               value="<?= old('monitoring_tool', $application['monitoring_tool']) ?>" placeholder="e.g., Datadog, New Relic">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="peak_users" name="peak_users" 
                               value="<?= old('peak_users', $application['peak_users']) ?>" min="0">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="peak_transactions" name="peak_transactions" 
                               value="<?= old('peak_transactions', $metrics['peak_transactions'] ?? '') ?>" min="0">
                    </div>
                    <div class="col-12">
                        ...
                        <textarea class="form-control" id="business_impact" name="business_impact" 
                                  rows="3" placeholder="Describe impact on business if application is down..."><?= old('business_impact', $application['business_impact']) ?></textarea>
                    </div>
                    <div class="col-md-6">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="license_type" name="license_type" 
                               value="<?= old('license_type', $application['license_type']) ?>" maxlength="100">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="license_expiry" name="license_expiry" 
                               value="<?= old('license_expiry', $application['license_expiry']) ?>">
                    </div>
                    <div class="col-md-6">
                        <label for="vendor_name" class="form-label">Vendor Name</label>
                        <input type="text" class="form-control" id="vendor_name" name="vendor_name" 
                               value="<?= old('vendor_name', $licensing['vendor_name'] ?? '') ?>" maxlength="150">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="vendor_contract_ref" name="vendor_contract_ref" 
                               value="<?= old('vendor_contract_ref', $application['vendor_contract_ref']) ?>" maxlength="255">
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="annual_cost" name="annual_cost" 
                                   value="<?= old('annual_cost', $application['annual_cost']) ?>" min="0" step="0.01">
                        </div>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control" id="cloud_subscription_details" name="cloud_subscription_details" 
                                  rows="3" placeholder="Details about cloud subscription (if applicable)..."><?= old('cloud_subscription_details', $application['cloud_subscription_details']) ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-2 mt-5 mb-5">
                <div class="col-12">
                    ...
                    <a href="<?= site_url('applications/details/' . $application['id']) ?>" class="btn btn-info btn-lg">
